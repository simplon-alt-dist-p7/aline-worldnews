...
export class ArticleService {
  ...
  async createArticle(data: CreateArticleDTO): Promise<Article> {
    if (!data.title || data.title.trim().length === 0) throw new ValidationError('Le titre est requis');
    if (data.title.length > 300) throw new ValidationError('Le titre ne peut pas dépasser 300 caractères');
    if (!data.subtitle || data.subtitle.trim().length === 0) throw new ValidationError('Le sous-titre est requis');
    if (data.subtitle.length > 300) throw new ValidationError('Le sous-titre ne peut pas dépasser 300 caractères');
    if (!data.subhead || data.subhead.trim().length === 0) throw new ValidationError('Le chapeau est requis');
    if (data.subhead.length > 1000) throw new ValidationError('Le chapeau ne peut pas dépasser 1000 caractères');
    if (!data.body || data.body.trim().length === 0) throw new ValidationError('Le contenu est requis');
