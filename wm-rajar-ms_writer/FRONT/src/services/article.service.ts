...
export const articleService = {
  ...
  createArticle: async (data: CreateArticleDTO): Promise<{ message: string; data: Article }> => {
    const payload = {
      title: data.title,
  ...
  updateArticle: async (
    ...
    return api.patch<{ message: string; data: Article }>(`/articles/${id}`, payload);
  },

  deleteArticle: async (id: number): Promise<{ message: string; data: Article }> => {
    return api.patch<{ message: string; data: Article }>(`/articles/${id}/delete`, {});
  },

  restoreArticle: async (id: number): Promise<{ message: string; data: Article }> => {
    return api.patch<{ message: string; data: Article }>(`/articles/${id}/restore`, {});
  },
};
